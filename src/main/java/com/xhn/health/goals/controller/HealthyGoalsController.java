...
public class HealthyGoalsController {

    @Autowired
    private HealthyGoalsService healthyGoalsService;

    @PostMapping
    @Operation(summary = "新增健康目标")
    public Mono<ResponseResult<Boolean>> add(
            @RequestBody HealthyGoals healthyGoals
    ) {
        return SecurityUtils.getCurrentUserId()
                .map(userId -> {
                    healthyGoals.setUserId(userId);
                    boolean result = healthyGoalsService.save(healthyGoals);
    ...
    public Mono<ResponseResult<Boolean>> delete(
        ...
        return SecurityUtils.getCurrentUserId()
                .flatMap(userId -> {
                    HealthyGoals goal = healthyGoalsService.getById(id);
                    if (goal == null) {
                    ...
                    boolean result = healthyGoalsService.removeById(id);
    ...
    public ResponseResult<Boolean> update(
        ...
        boolean result = healthyGoalsService.updateById(healthyGoals);
        return result ? ResponseResult.success(true) : ResponseResult.error("修改失败");
    }

    @GetMapping("/{id}")
    @Operation(summary = "根据ID查询健康目标")
    public ResponseResult<HealthyGoals> getById(
            @Parameter(description = "目标ID") @PathVariable Long id
    ) {
        HealthyGoals healthyGoals = healthyGoalsService.getById(id);
        return healthyGoals != null ? ResponseResult.success(healthyGoals) : ResponseResult.error("查询失败");
    }

    @GetMapping
    @Operation(summary = "查询所有健康目标列表")
    public ResponseResult<List<HealthyGoals>> list() {
        List<HealthyGoals> list = healthyGoalsService.list();
        return ResponseResult.success(list);
    }

    @GetMapping("/my")
    @Operation(summary = "获取我的健康目标列表")
    public Mono<ResponseResult<List<HealthyGoals>>> getMyGoals(
            @Parameter(description = "状态") @RequestParam(required = false) String status,
            @Parameter(description = "目标类型") @RequestParam(required = false) String goalType
    ) {
        return SecurityUtils.getCurrentUserId()
                .map(userId -> {
                    List<HealthyGoals> goals;
                    if (status != null && !status.isEmpty()) {
                        goals = healthyGoalsService.getGoalsByUserIdAndStatus(userId, status);
                    } else if (goalType != null && !goalType.isEmpty()) {
                        goals = healthyGoalsService.getGoalsByUserIdAndType(userId, goalType);
                    } else {
                        goals = healthyGoalsService.getGoalsByUserId(userId);
                    }
    ...
    public ResponseResult<Page<HealthyGoals>> page(
            @Parameter(description = "页码") @RequestParam int pageNum,
            @Parameter(description = "每页数量") @RequestParam int pageSize
    ) {
        Page<HealthyGoals> page = new Page<>(pageNum, pageSize);
        Page<HealthyGoals> resultPage = healthyGoalsService.page(page);
